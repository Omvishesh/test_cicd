...
def get_reference_url(ref: str, source: str = '') -> str:
    base_rbi_docs_url = 'https://website.rbi.org.in/documents/87730/30842423/'
    ...
    if re.match(r"Inflation Expectations Survey of Households \w+ \d{4}", ref):
        ...
        if source and source.endswith('.pdf'):
            return f'{base_minutes_url}{source.replace(" ", "+")}'
        else:
        ...
        if source and source.endswith('.pdf'):
            return f'https://dea.gov.in/sites/default/files/{source.replace(" ", "%20")}'
        else:
            return "https://dea.gov.in/monthly-economic-report-table"
    elif re.match(r"RBI Bulletin \w+ \d{4}", ref):
        if source and source.endswith('.pdf'):
            return f"https://rbidocs.rbi.org.in/rdocs/Bulletin/PDFs/{source}"
        else:
            return "https://rbi.org.in/Scripts/BS_ViewBulletin.aspx"
    elif re.match(r"RBI State Finances \w+ \d{4}", ref):
        if source and source.endswith('.pdf'):
            return f"https://rbidocs.rbi.org.in/rdocs/Publications/PDFs/{source}"
        else:
        ...
        if source and source.endswith('.pdf'):
            return f"https://rbidocs.rbi.org.in/rdocs/AnnualReport/PDFs/{source}"
        else:
        ...
        if source and source.endswith('.pdf'):
            return f"https://rbidocs.rbi.org.in/rdocs/PressRelease/PDFs/{source}"
        else:
        ...
        if source and source.endswith('.pdf'):
            return f"https://rbidocs.rbi.org.in/rdocs/notification/PDFs/{source}"
        else:
        ...
        if source and source.endswith('.pdf'):
            return f"https://rbidocs.rbi.org.in/rdocs/PublicationReport/PDFs/{source}"
        else:
        ...
        if source and source.endswith('.pdf'):
            return f"https://rbidocs.rbi.org.in/rdocs/Speeches/PDFs/{source}"
        else:
        ...
        if source and source.endswith('.pdf'):
            return f"https://www.indiacode.nic.in/bitstream/123456789/{source}"
        else:
        ...
        if source and source.endswith('.pdf'):
            return f'https://rbidocs.rbi.org.in/rdocs/content/pdfs/{source}'
        else:
            return 'https://rbi.org.in/Scripts/Regulations.aspx'
    elif re.match(r'.*Rules, \d{4}$', ref):
        if source and 'indiacode.nic.in' in source:
            return source
    ...
    # RBI Governor Speeches / Interviews / Press Conferences / Fireside Chats
        ...
        if source.lower().endswith('.pdf') and source.startswith("https://www.dcmsme.gov.in/"):
            return source
        else:
